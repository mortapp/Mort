-- Keep a perpetual RevenueCat entitlement open-ended when another active
-- product has a finite renewal date. The Edge Function remains the validated
-- service-only entry point for this RPC.
...
begin
  ...
  if v_state_changed then
    select
      ...
      case
        when bool_or(state.active_until is null) then null
        else max(state.active_until)
      end
...
-- Reconcile caches written before this correction. Only active, perpetual
-- provider states qualify; revoked lifetime products do not.
with perpetual_users as (
  select distinct state.user_id
  from public.revenuecat_product_states state
  where state.active
    and state.active_until is null
)
update public.monetization_entitlements_cache cache
set active_until = null,
    refreshed_at = now()
from perpetual_users
where cache.user_id = perpetual_users.user_id
  and cache.source = 'revenuecat'
  and cache.active_until is not null;
...
update public.user_subscription_status status
set current_period_ends_at = null
from perpetual_users
where status.user_id = perpetual_users.user_id
  and status.source = 'revenuecat'
  and status.current_period_ends_at is not null;
