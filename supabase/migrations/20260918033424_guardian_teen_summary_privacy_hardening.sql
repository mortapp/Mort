...
begin
  ...
  -- Guardian Mode may show only incident status that the safety system has
  -- explicitly shared with this guardian. A link alone never reveals the
  -- existence of a private report about the teen.
  select count(distinct incident.id)::integer
  into safety_alerts
  from public.safety_incidents incident
  join public.incident_participants participant
    on participant.incident_id = incident.id
   and participant.user_id = guardian
   and participant.can_view_status is true
