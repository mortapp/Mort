-- Corrected support AI classifier function.
-- The previous migration had an unbalanced regex. This migration replaces it
-- with a simplified, validated version.
...
begin
  if v_message ~ 'suicid|kill myself|kill me|hurt myself|self.?harm|kidnap|abduct|traffick|immediate danger|being followed|sexual assault|rape|won.?t let me leave|trapped at.*job|child pornography|underage nude|end(ing)? my life|want(ing)? to die|no reason to live|i do not want to live|i do not want to be alive|i am in immediate danger|there is a (gun|knife|weapon)|someone.*has a weapon|someone.*pulled.*knife|a person.*weapon' then
    v_level := 3;
    v_band := 'urgent';
