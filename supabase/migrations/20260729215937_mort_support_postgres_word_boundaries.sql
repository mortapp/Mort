-- PostgreSQL ARE uses \m and \M for word boundaries; \b is a backspace.
-- Keep the database classifier behavior aligned with the Edge fallback.
...
begin
  if v_message ~ '(suicid|kill myself|kill me|hurt myself|self.?harm|kill (him|her|them|someone)|kidnap|abduct|traffick|weapon|gun|knife|immediate danger|being followed right now|sexual assault|rape|won.?t let me leave|trapped at (the )?job|\mcsam\M|child pornography|underage nude)' then
    v_level := 3;
    v_category := 'immediate_safety';
    v_intent := 'safety_emergency';
    v_action := 'safety_center';
  elsif v_message ~ '(threat|stalk|harass|blackmail|extort|nude|sexual message|meet.*alone|keep (this|it) (a )?secret|don.?t tell ((your|my|the) )?(parent|guardian)|off.platform|cashapp|gift card|verification code|\mpin\M|(start|finish|end) (code|pin)|password|social security|\mssn\M|passport|driver.?s license|card number|\mcvc\M|\mcvv\M|exact (home )?address|unsafe at (the )?job|scam|fraud|ignore.*instruction|system prompt|developer message|service.?role|another user|other user.?s|database rows|dump.*table|show.*transcript)' then
    v_level := 2;
