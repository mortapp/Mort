...
## Android release signing recovery and packaging -- resolved (2026-09-15)

Recovered the existing MORT upload keystore (never regenerated) from
`C:\Users\micha\MortSecrets\android\mort-upload-key.jks`, at the exact path
documented in `docs/mobile/MORT_UPLOAD_CERTIFICATE_REPORT.md`. Proved
continuity from the public certificate alone (SHA-256
`04:42:C2:21:38:B0:D6:23:F9:A6:F4:78:1A:44:2B:F4:A9:33:27:8F:AB:8E:85:76:74:4D:C1:FD:7C:33:4D:EF`,
matching the documented record exactly) before ever touching the
DPAPI-protected store/key passwords. Decrypted those passwords in-memory via
`Import-Clixml` (the project's own documented recovery mechanism, tied to
this Windows user account) for the build subprocess only -- never printed or
persisted. Along the way, fixed a real but unrelated Flutter tooling issue:
`GeneratedPluginRegistrant.java` unconditionally referenced the dev-only
`integration_test` plugin's class regardless of build type, while Flutter's
own dependency wiring correctly excludes dev dependencies from the release
compile classpath (confirmed in Flutter SDK source,
`flutter_tools/lib/src/flutter_plugins.dart`) -- resolved by letting
Flutter's Gradle-triggered codegen regenerate the registrant in release mode
rather than reusing one generated by a plain `pub get`.

Produced real signed release artifacts: `app-release.apk` (75.8 MB,
SHA-256 `8cc8e8a8ac055098a0f3eff8fedf6c21d299d53b80fdfbbda65c5ba50485d2a5`) and
`app-release.aab` (73.4 MB, SHA-256
`f55ec4938b6a02b5dab68714523979c3fc7653e050c03cb07d3be3274e39704f`), both
verified via `apksigner`/`jarsigner` to carry the exact continuity-proven
`CN=MORT Upload Key` signer, package `com.mortapp.mobile`, version
`0.9.16+107`, `ELF_16KB_ALIGNMENT=PASS` (18 native libraries). Copies
preserved at `C:\Users\micha\Downloads\MORT-0.9.16+107-release.{apk,aab}`
with matching checksums. `PLAY_VERSION_CODE_CHECK` remains `EXTERNAL`: the
repo's own `docs/mobile/MORT_VERSIONING_POLICY.md` states local source
cannot prove Play Console's remote version history, and no Play Developer
API credentials exist in this environment.

## AI Safety edge function -- deployed and verified (2026-09-15)

Deployed the repository's own dedicated `supabase/functions/ai-safety/index.ts`
(plus its `_shared/observability.ts` dependency) to the `ai-safety` slug,
replacing a generic alias (`serveSupportFunction("safety-triage")`) that had
drifted onto that slug at some point independent of this session's other
work -- `HOSTED_CONFIG_DRIFT`, not a product bug, harness bug, provider
gate, or stale expectation. Confirmed via repo-wide grep and an independent
security review (fresh, non-forked subagent) that zero live callers
anywhere -- Flutter client, other edge functions, migrations -- invoke the
`ai-safety` slug by name; the real, actively-used safety-triage path for
real users runs through the separate `support-safety-triage` slug, which
this deployment does not touch. All database objects the dedicated
implementation needs (the `client_request_id` column and its partial unique
index, `service_role`/`authenticated` grants, the `ai_safety_scan`
rate-limit entry) already existed live, built in July before this drift
occurred.

Pre-deployment: reconfirmed project identity (`Mort` /
`rakjydmgwwgtdislanbt` / `ACTIVE_HEALTHY`), captured the prior hosted
version (18, sha `efda36e2...`) and `support-safety-triage`'s baseline
(version 10, sha `1a0ea308...`) as rollback/comparison evidence. Deployed via
`deploy_edge_function` (version 18 -> 19). Post-deployment: re-fetched the
hosted source and confirmed it is byte-for-byte identical to the repository
files; confirmed `support-safety-triage` and every other function's
version/sha were completely unchanged.

Running `scripts/qa-ai-safety-edge.mjs` surfaced a genuine, unrelated
environment fault: this machine currently has no IPv6 default route at all
(confirmed independently via `Get-NetRoute`/`Test-NetConnection`, not a
Node or DNS bug), which broke every QA script's direct-Postgres-connection
fixture setup against Supabase's IPv6-only `db.<ref>` host. Fixed
`scripts/feature-qa-helpers.mjs` with two narrow, general QA-tooling
improvements: (1) resolve the direct host via `dns.resolve6`/`resolve4`
directly rather than relying on `pg`'s internal `dns.lookup()`, since
Windows' `getaddrinfo` was filtering out the AAAA record `dns.resolve6`
returns correctly; (2) fall back to Supabase's IPv4 Supavisor pooler
(`aws-0`/`aws-1-us-east-2.pooler.supabase.com`, user
`postgres.<project-ref>`) only on a network-layer connection failure, never
on an auth failure -- so networks with working IPv6 are unaffected and
networks without it (like this one) still work. With that fix,
`qa-ai-safety-edge.mjs` ran end-to-end for real against the live deployed
function: authenticated deterministic scan, idempotent replay, cross-user
isolation, and oversized-payload rejection all passed, and its own cleanup
removed the QA users it created. One synthetic `ai_moderation_events` row
(`user_id` already `null` after its owning QA user's deletion) was left
behind by the scenario itself rather than its cleanup step; deleted directly
afterward. Final state: `HOSTED_SOURCE_MATCHES_REPOSITORY=YES`,
`QA_AI_SAFETY_EDGE=PASS`, `SUPPORT_SAFETY_TRIAGE_UNCHANGED=YES`,
`REAL_USER_ROWS_MUTATED=0`.

## Remaining verification before acceptance

Final backend regression / Flutter gate only if backend changes affect
client tests (this AI Safety deployment is backend-only; no Flutter client
source changed, so no rebuild is required); Play Console upload of the
signed release artifacts (external, requires the adult account owner);
`PLAY_VERSION_CODE_CHECK=EXTERNAL` confirmation before that upload; iOS
BrowserStack run result; local database regression (Docker engine returned
HTTP 500 in the prior session -- no safe local substitute was established,
and production was correctly not used as a stand-in); final scorecard sync.

Support and AI Safety backend work, guardian financial-summary deployment,
and the Android signed release candidate are now all closed. P0/P1 zero and
technical 100% are **not yet fully established** pending the external gates
above (Play Console upload/version-code confirmation, iOS evidence currency,
local DB regression substitute).
