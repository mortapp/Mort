-- Preserve Stripe's requires_action state in the normalized payment state machine.
-- Hosted migration: 20260918120347_mort_stripe_requires_action_state_v1.
...
begin
  ...
  if p_current in ('SUCCEEDED', 'DECLINED', 'FAILED', 'CANCELLED', 'DUPLICATE_BLOCKED') then
    if p_observed = p_current then return p_current; end if;
    raise exception 'payment_state_regression';
  end if;

  if p_current = 'READY' and p_observed in (
    'PROCESSING', 'REQUIRES_ACTION', 'CANCELLED', 'PROVIDER_UNAVAILABLE'
  ) then return p_observed; end if;
  ...
  if p_current = p_observed then return p_current; end if;
...
begin
  ...
  next_state := case
    when payment.normalized_state = 'READY' and observed_state = 'SUCCEEDED' then 'SUCCEEDED'
...
revoke all on function private.reduce_stripe_payment_state_v1(text, text)
  from public, anon, authenticated;
grant execute on function private.reduce_stripe_payment_state_v1(text, text)
  to service_role;
