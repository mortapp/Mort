...
const routeSummaries = {
  '/privacy/': 'How MORT collects, uses, protects, retains, and deletes information.',
  '/terms/': 'Core service terms for MORT.',
  '/terms-of-use/': 'Rules for using MORT accounts, jobs, messages, and work coordination.',
  ...
  '/payment-disputes/': 'How MORT records payment obligations and disagreements without processing money.',
...
<body>
  ...
  <header class="topbar"><a class="brand" href="/"><img src="/mort-mark.svg" width="28" height="28" alt="">MORT <small>Legal &amp; safety</small></a><span class="sp"></span><span class="draft">Draft — pending legal review</span><a class="back" href="https://mortapp.org">&larr; Back to MORT</a></header>
  <div class="wrap">
...
${blocker}
      <div class="callout"><b>Current limits:</b> MORT is a 13+ local-work coordination service with account eligibility restrictions. It does not guarantee identity, safety, jobs, or payment; it does not process real-world job payments; and identity verification is not currently available.</div>
      ${body}
    </main>
  </div>
  <footer class="foot"><div class="row"><span><b>MORT</b> &nbsp;&middot;&nbsp; Publisher: ${publisher}</span><span>Support: ${supportEmail}</span><span>Effective: ${effectiveDate}</span><span>Website: ${websiteUrl}</span></div><div class="row" style="margin-top:8px"><span>Draft — pending qualified legal review. This is public information, not legal advice.</span></div></footer>
  ${scripts}
...
const pages = {
  'index.html': page({
    ...
    body: `<section class="sec"><h2>What MORT is today</h2><p>MORT is a 13+ service for local work coordination. Public visitors can learn about the mission and safety model, but jobs, participant messaging, private locations, contracts, and active-job workflows require server-approved account eligibility.</p></section><div class="hubgrid">${routes.slice(1).map(([href, label]) => `<a class="glass hubcard" href="${href}"><h3>${label}</h3><p>${routeSummaries[href]}</p><span class="arw">Read &rarr;</span></a>`).join('')}</div>`,
  }),
