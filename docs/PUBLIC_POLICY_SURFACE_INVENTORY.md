...
1. **No real public policy URL exists today.** The public web site (surface 1) is fully built and CI-validated but never deployed. Play Console needs a real HTTPS Privacy/Deletion/Child-Safety URL -- this is an `OWNER_ACTION_REQUIRED` (deploy target + the six `MORT_PUBLIC_*` env vars), not something fixable purely by writing better copy.
