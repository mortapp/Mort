...
## Refined role taxonomy (every classified relationship, re-sorted)

The SUBJECT/ACTOR/SHARED/HOLDER labels used in the table above collapse into
nine more precise roles. Re-sorting all 99 classified relationships by role
confirms each role resolves to exactly one disposition (with one deliberate
split, explained under billing/provider subject) — no role was found to need
mixed treatment across the columns that carry it.

| Role | Members | Disposition |
|---|---|---|
| **Live authorization holder** — the row itself IS the grant; must not survive as a null-holder ghost | `team_role_assignments.user_id`, `admin_role_assignments.user_id`, `partner_memberships.user_id`, `partner_staff.user_id`, `stripe_financial_role_assignments.user_id`, `support_staff_assignments.user_id`, `guardian_connections.teen_id`, `guardian_connections.guardian_id` | **CASCADE / DELETE** (class A) |
| **Primary subject** — the record is fundamentally about this person | `account_ban_appeals.user_id`, `appearance_review_cases.subject_user_id`, `document_capture_sessions.subject_user_id`, `document_web_reuse_requests.subject_user_id`, `identity_verification_evidence.user_id`, `live_presence_challenges.subject_user_id`, `teen_abandonment_reports.teen_id`, `support_ticket_appeals.requester_id`, `support_attachments.owner_id`, `support_evidence_attachments.owner_id` | **SET NULL** (class B) |
| **Reporter** — flagged/reported something | `teen_abandonment_reports.reported_by_adult_id`, `message_safety_evidence.sender_id` | **SET NULL** (class B) |
| **Reviewer / moderator** — reviewed or decided on someone else's case | `appearance_review_assignments.reviewer_id`, `appearance_review_decisions.reviewer_id`, `document_review_assignments.reviewer_id`, `document_review_decisions.reviewer_id`, `document_vault_access_grants.reviewer_id`, `incident_outcomes.decided_by`, `incident_preservation_orders.ordered_by`, `payment_dispute_assignments.reviewer_id`, `payment_dispute_decisions.reviewer_id`, `payment_dispute_appeals.reviewed_by`, `team_conflict_disclosures.reviewed_by`, `team_device_compliance.reviewed_by`, `teen_abandonment_reports.decided_by`, `stripe_payment_resolutions.reviewer_id`, `stripe_payment_resolutions.financial_operator_id` | **SET NULL** (class B) |
| **Historical actor** — performed a one-time action on a record; includes staff `assigned_by`/`approved_by`/`revoked_by` trails and the closed-out remnant of `incident_assignments.assigned_to` | `incident_actions.actor_id`, `incident_contact_attempts.actor_id`, `job_execution_cancellations.actor_id`, `safety_cancellations.actor_id`, `payment_dispute_timeline.actor_id`, `support_internal_notes.author_id`, `payment_dispute_statements.author_id`, `job_completion_assertions.asserted_by`, `completion_evidence_records.submitted_by`, `payment_dispute_evidence.submitted_by`, `payment_confirmation_records.confirmed_by`, `job_contract_change_requests.requested_by`, `job_contract_versions.created_by`, `partner_invite_codes.created_by`, `partner_permissions.granted_by`, `first_party_trust_control.enabled_by`, `poster_payment_restrictions.imposed_by`, `poster_payment_restrictions.lifted_by`, `payment_evidence_export_events.requested_by`, `incident_assignments.assigned_by`, `incident_assignments.assigned_to` (after close-out), `appearance_review_assignments.assigned_by`, `document_review_assignments.assigned_by`, `payment_dispute_assignments.assigned_by`, `job_arrival_handshakes.finish_confirmed_by`/`.finish_requested_by`/`.start_confirmed_by`, `team_role_assignments.approved_by`/`.revoked_by`, `team_training_completions.approved_by`, `stripe_job_payment_attempts.initiated_by`, `stripe_financial_role_assignments.assigned_by`, `support_staff_assignments.assigned_by` | **SET NULL** (class B) |
| **Shared participant** — a bilateral/multilateral record (job, contract, dispute) that must survive for the counterparty | `jobs.poster_id`, `applications.teen_id`, `job_contracts.adult_id`, `job_contracts.teen_id`, `job_contract_acceptances.user_id`, `job_contract_change_acceptances.user_id`, `job_payment_obligations.obligated_poster_id`, `job_payment_obligations.worker_id`, `payment_disputes.poster_id`, `payment_disputes.worker_id`, `payment_disputes.opened_by`, `payment_dispute_appeals.appellant_id`, `guardian_connection_audit_events.teen_id` | **SET NULL** (class B), with `jobs.poster_id` additionally close-out (unfinished job closed, non-terminal application terminalized before deidentification) |
| **Billing / provider subject** — has or had a Stripe-side identity | `stripe_connected_accounts.user_id`, `stripe_customers.user_id`, `stripe_job_payment_intents.adult_id`, `stripe_job_payment_intents.teen_id` | **EXPLICIT HOLD, then SET NULL** (class D — the three-table hold in `private.stripe_financial_retention_required` pauses deletion before these are ever reached) |
| | `stripe_account_onboarding_sessions.user_id` | **SET NULL** (class B — attempted/incomplete onboarding, not an active relationship, not covered by the hold) |
| **Legal acceptance subject** — accepted, declined, or owes re-acceptance of a policy version | `legal_acceptances.user_id`, `legal_declines.user_id`, `legal_reacceptance_requirements.user_id`, `legal_acceptance_audit_events.user_id` | **SET NULL** (class B), `LEGAL_REVIEW_REQUIRED` for whether a pseudonymous key should replace plain SET NULL |
| **Ordinary disposable user data** — no independent value once the user is gone | *(none of the 99 pinned relationships — by construction, anything in this role was already correct `ON DELETE CASCADE` before this session and needed no classification work; it's the bulk of the schema's other ~237 direct-user foreign keys: `account_security_events`, `account_security_preferences`, `ad_frequency_caps`, most `*_preferences` tables, etc.)* | **CASCADE / DELETE** (pre-existing, unaffected) |

No role produced a mixed disposition except **billing/provider subject**,
which splits by whether the specific table is one of the three the existing
hold mechanism checks — and that split is the hold's existing design, not a
new inconsistency introduced here. **Exceptional delete blocker (class E)
has zero members** — no relationship in this schema was found to need
RESTRICT retained.
