-- Deterministic safety triage bands with contextual emergency matching.
-- Ordinary mentions of tools must not be treated as active emergencies.
...
begin
  if v_message ~ '(suicid|kill myself|kill me|hurt myself|self.?harm|kill (him|her|them|someone)|kidnap|abduct|traffick|immediate danger|being followed right now|sexual assault|rape|won.?t let me leave|trapped at (the )?job|\mcsam\M|child pornography|underage nude|someone (has|brought|pulled out|pointed) (a )?(gun|knife|weapon)|there is (a )?(gun|knife|weapon) (here|at)|threaten.{0,30}(gun|knife|weapon)|(gun|knife|weapon).{0,30}(pointed|attacked|threat|scared))' then
    v_level := 3;
    v_band := 'urgent';
...
'Deterministic non-diagnostic support triage: routine, concern, serious, or urgent. Urgent bypasses AI-provider handling.';
