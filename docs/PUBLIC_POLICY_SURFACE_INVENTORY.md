...
**Vercel access checked (2026-08-29):** the only Vercel account reachable from this session (team `mortapphelp-7067s-projects`, matching the `mortapp.help@gmail.com` contact) has exactly one project, `loop` -- linked to a completely different GitHub repository (`mortapp/Loop`) with completely different domains (`loop-teal-rho.vercel.app` and similar). No project serving `mort-web.vercel.app` is reachable from this account. This means either (a) that domain is hosted under a different Vercel account/team not connected to this session, or (b) a different hosting provider entirely. **`OWNER_ACTION_REQUIRED`: connect the Vercel account/team that actually owns `mort-web.vercel.app` to this session (or provide its project ID/team ID), or deploy `web/public/` there manually.** Per explicit instruction, no new Vercel project was created as a substitute -- the generated package (`web/public/`, CI-validated, ready) is waiting for a real deploy target.
