...
$stageRoot = Join-Path $stageParent 'package-0.9.5'
...
$sourceStage = Join-Path $stageRoot 'mort-android-0.9.5-final-source-clean'
New-Item -ItemType Directory -Force -Path $sourceStage | Out-Null
foreach ($file in Get-ChildItem -LiteralPath $root -Recurse -File -Force) {
  if (-not (Test-SourceFileAllowed $file)) { continue }
  Copy-RelativeFile -RelativePath $file.FullName.Substring($root.Length + 1) -DestinationRoot $sourceStage
}
$sourceZip = Join-Path $artifacts 'mort-android-0.9.5-final-source-clean.zip'
...
$googleZip = New-SelectedPackage -Name 'mort-google-auth-evidence-0.9.5' -Files @(
...
$stripeZip = New-SelectedPackage -Name 'mort-stripe-testmode-evidence-0.9.5' -Files @(
...
$securityZip = New-SelectedPackage -Name 'mort-security-evidence-0.9.5' -Files @(
...
$supportZip = New-SelectedPackage -Name 'mort-support-admin-jobs-pin-evidence-0.9.5' -Files @(
...
$playZip = New-SelectedPackage -Name 'mort-play-review-package-0.9.5' -Files @(
  'docs\MORT_0_9_5_FINAL_REPORT.md',
  'docs\MORT_ANDROID_EMULATOR_EVIDENCE_0_9_5.md',
  'docs\MORT_COMPLETION_SCORE_0_9_5.md',
  'scripts\android-lint-release.ps1',
  'scripts\qa-android-apk.ps1',
  'scripts\verify-play-aab.ps1'
) -Trees @('docs\play-final')

$docsZip = New-SelectedPackage -Name 'mort-documentation-0.9.5' -Trees @('docs')

$apk = Join-Path $artifacts 'mort-android-0.9.5-final-qa.apk'
$aab = Join-Path $artifacts 'mort-android-0.9.5-closed-test.aab'
...
$records = @(
  $sourceZip, $apk, $aab, $googleZip, $stripeZip, $securityZip, $supportZip,
  $playZip, $docsZip
) | ForEach-Object { Get-ArtifactRecord -Path $_ }

$inventoryJson = Join-Path $artifacts 'MORT_0_9_5_ARTIFACT_INVENTORY.json'
$inventoryMarkdown = Join-Path $artifacts 'MORT_0_9_5_ARTIFACT_INVENTORY.md'
...
$markdown = @(
  '# MORT 0.9.5 Artifact Inventory',
  '',
  'Generated from the verified 0.9.5 closed-pilot tree. This is not a production-readiness claim.',
  '',
