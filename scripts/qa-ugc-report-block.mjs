...
const block = await teen.client.rpc('block_user_v2', {
  p_blocked_id: adult.user.id,
  p_client_request_id: randomUUID(),
});
assert(!block.error && block.data?.ok === true, 'Teen could not block the adult fixture.');
const blockedSend = await adult.client.rpc('send_safe_message_v2', {
  p_thread_id: threadId,
  p_body: 'This message must be rejected while blocked.',
  p_client_request_id: randomUUID(),
});
...
await teen.client.rpc('unblock_user', { p_blocked_id: adult.user.id });
