...
export default function GuardianMessagesScreen() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    async function loadThreads() {
      try {
        const rows = await listThreadsForUser(userId, "guardian");
        if (!active) return;
        setThreads(rows);
        setError(null);
      } catch (err) {
  ...
  return (
    <Screen>
      <Text variant="title">Supervised chat</Text>
      <ErrorBanner message={error} />
      {threads.length === 0 ? <EmptyState title="No supervised threads" body="Threads appear when connected teens apply to jobs." /> : null}
      {threads.map((thread) => (
        <Card key={thread.id}>
